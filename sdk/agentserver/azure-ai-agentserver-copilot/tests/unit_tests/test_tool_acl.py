...
# ---------------------------------------------------------
"""Unit tests for the ToolAcl engine (tool_acl.py)."""
...
from azure.ai.agentserver.copilot.tool_acl import ToolAcl, _Rule
...
class TestFromEnv:
    ...
    def test_custom_env_var_name(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        p = tmp_path / "acl.yaml"
        p.write_text('version: "1"\nrules: []\n')
...
class TestShellRules:
    ...
    def test_regex_is_search_not_fullmatch(self) -> None:
        """re.search semantics — pattern can appear anywhere in the string."""
        acl = _acl_from_yaml(
            """\
            version: "1"
            default_action: allow
...
class TestEdgeCases:
    ...
    def test_rule_without_when_key_null_matches(self) -> None:
        """A rule with ``when: null`` (or omitted) should match every request of its kind."""
        acl = _acl_from_yaml(
            """\
            version: "1"
            default_action: deny
    ...
    def test_sample_yaml_file_is_parseable(self) -> None:
        """The bundled sample ACL file must parse without errors."""
        sample = (
        ...
        if not sample.exists():
            pytest.skip("tools_acl.yaml not found")
        acl = ToolAcl.from_file(sample)
        assert acl is not None
